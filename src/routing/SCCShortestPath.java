...
/**
 * Descreve o algoritmo de roteamento Spectrum Continuity
 * and Contiguity ShortestPath.
 * @author Andr� 
 */
public class SCCShortestPath implements IRoutingAlgorithm { // NOPMD by Andr� on 19/06/17 13:23
	/**
	 * Inst�ncia da Classe.
	 * @author Andr� 			
	 */	
	private static final SCCShortestPath SCCSP_INSTANCE = new SCCShortestPath();
	/**
	 * M�todo para encontrar uma rota baseado na continuidade
	 * e contiguidade dos slots.
	 * @author Andr� 			
	 */	
	public RoutingAlgorithmSolution findRoute(final OpticalLink[][] network, final CallRequest callRequest, final List<OpticalSwitch> listOfNodes){ // NOPMD by Andr� on 19/06/17 13:22
		...
		final int reqNumbOfSlots = function.calculateNumberOfSlots(ModulationLevelType.FOUR_QAM, callRequest);
		
		final List<ArrayList<Integer>> continuity = new ArrayList<>();
		
		for(int y=0;y<numNodes;y++){
			final ArrayList<Integer> temp = new ArrayList<Integer>();  // NOPMD by Andr� on 19/06/17 13:14
			continuity.add(temp);			
		}
		
		for(int y=0;y<numberOfSlots;y++){
			continuity.get(idSourceNode).add(1); // NOPMD by Andr� on 19/06/17 13:15
		}		
		...
		while (!listQ.isEmpty()) {
			...
			if (totalShortDist != -1) {
				for (cont = 0; cont < numNodes; cont++) {
					...
					if(callRequest.getCallRequestType().equals(CallRequestType.BIDIRECTIONAL)){ // NOPMD by Andr� on 19/06/17 13:15
						
						if(network[totalShortDist][cont] != null && network[cont][totalShortDist] != null){
							
							double distBetwNodes = Double.MAX_VALUE; // NOPMD by Andr� on 19/06/17 13:19
							final ArrayList<Integer> temp2 = new ArrayList<Integer>(); // NOPMD by Andr� on 19/06/17 13:17
									
							if(network[totalShortDist][cont].getLength()!=Double.MAX_VALUE && network[cont][totalShortDist].getLength()!=Double.MAX_VALUE){ // NOPMD by Andr� on 19/06/17 13:17
											
								//Verifica o espectro da combina��o de fibras
								for(int f=0;f<numberOfSlots;f++){
									if(network[totalShortDist][cont].availableSlot(f) && network[cont][totalShortDist].availableSlot(f) && continuity.get(totalShortDist).get(f) == 1){ // NOPMD by Andr� on 19/06/17 13:17
										temp2.add(1);
									}else{
										temp2.add(0);
									}	
								...
								final double linkCost = function.evaluateInverseCFSA(temp2, reqNumbOfSlots, numberOfSlots);
																					
								if(linkCost<Double.MAX_VALUE){ // NOPMD by Andr� on 19/06/17 13:20
						...
						if(network[totalShortDist][cont] != null){
							...
							double distBetwNodes = Double.MAX_VALUE;
							final ArrayList<Integer> temp2 = new ArrayList<Integer>();	// NOPMD by Andr� on 19/06/17 13:22

							if(network[totalShortDist][cont].getLength()!=Double.MAX_VALUE){ // NOPMD by Andr� on 19/06/17 13:22
										
								//Verifica o espectro da combina��o de fibras
								for(int f=0;f<numberOfSlots;f++){
									if(network[totalShortDist][cont].availableSlot(f) && continuity.get(totalShortDist).get(f) == 1){ // NOPMD by Andr� on 19/06/17 13:22
										temp2.add(1);
									}else{
										temp2.add(0);
									}	
		...
		if(!track.isEmpty()){
			...
			for(int i=1; i<index;i++){
				...
				if(callRequest.getCallRequestType().equals(CallRequestType.BIDIRECTIONAL)){ // NOPMD by Andr� on 16/06/17 10:17
	 ...
	 * @return O objeto SCCSP_INSTANCE
	 * @author Andr� 			
	 */	
	public static SCCShortestPath getSCCSPInstance(){
		return SCCSP_INSTANCE;
	}
