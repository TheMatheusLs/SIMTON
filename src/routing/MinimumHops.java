...
/**
 * Descreve o algoritmo de roteamento Minimum Hops.
 * @author Andr� 
 */
public class MinimumHops implements IRoutingAlgorithm { // NOPMD by Andr� on 19/06/17 09:32
	/**
	 * Inst�ncia da Classe.
	 * @author Andr� 			
	 */	
	private static final MinimumHops MH_INSTANCE = new MinimumHops();	
	/**
	 * M�todo para encontrar uma rota baseado no n�mero de saltos.
	 * @author Andr� 			
	 */	
	public List<RoutingAlgorithmSolution> findRoute(final OpticalLink[][] network, int originNode,  int destinationNode, final List<OpticalSwitch> listOfNodes) throws InvalidNodeIdException{ // NOPMD by Andr� on 19/06/17 09:31
				...
				for (cont = 0; cont < numNodes; cont++) {
					...
					if(parameters.getCallRequestType().equals(CallRequestType.BIDIRECTIONAL)){ // NOPMD by Andr� on 16/06/17 10:16
						...
						if(network[totalShortDist][cont] != null && network[cont][totalShortDist] != null){				
							...
							final double uplinkCost = function.getLengthInLink(optLinkUp);
							final double downlinkCost = function.getLengthInLink(optLinkDown);
								
							if(uplinkCost==Double.MAX_VALUE || downlinkCost==Double.MAX_VALUE){
						...
						if(network[totalShortDist][cont] != null){							
							...
							final OpticalLink optLinkUp = network[totalShortDist][cont];							
							final double uplinkCost = function.getLengthInLink(optLinkUp);						
							if(uplinkCost==Double.MAX_VALUE){
								distBetwNodes = Double.MAX_VALUE;
							}else{
								distBetwNodes = 1.0;
							}
	 ...
	 * @return O objeto MH_INSTANCE
	 * @author Andr� 			
	 */	
	public static MinimumHops getMinimumHopsInstance(){
		return MH_INSTANCE;
	}
