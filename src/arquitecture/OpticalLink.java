package arquitecture; // NOPMD by Andr� on 29/05/17 11:42
...
public class OpticalLink {	 // NOPMD by Andr� on 29/05/17 11:42
	 ...
	 * @author Andr�
	 */
	public OpticalLink(final int opticalLinkId, final int source, final int destination, final int srlg, final double length) throws InvalidFiberLengthException {		
		final SimulationParameters parameters = new SimulationParameters();
		if(length<parameters.getSpanSize()){
			throw new InvalidFiberLengthException("Fiber length between nodes "+ source +" and "+ destination + " is invalid.", 1);
		}		
		...
		this.inicializePowersAndFrequencies(parameters.getNumberOfSlots());
	}
	/**
	 ...
	 * @author Andr� 			
	 */	
	private List<OpticalSpan> configureSpansInLink(final double length) throws InvalidFiberLengthException {
		
		final SimulationParameters parameters = new SimulationParameters();
		final double spanSize = parameters.getSpanSize();
		
		if(length%spanSize!=0){
			throw new InvalidFiberLengthException("The fiber size is not multiple of the span size.", 1);
		}
	...
	public void allocate(final int slot, final double initialPower){		
		...
		final SimulationParameters parameters = new SimulationParameters();
		final double muxGain = Math.pow(10, parameters.getMuxLoss()/10);
		...
		for(int i=0;i<this.spans.size();i++){
			final double dioLoss =  Math.pow(10, parameters.getDioLoss()/10);
			final double fiberGain = this.getSpanOpticalFiber(spans.get(i)).getLength()*parameters.getFiberAtenuationCoefficient();			
			signal *= dioLoss;			
