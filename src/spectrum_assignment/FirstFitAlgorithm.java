 ...
 * @author Andr� 
 */
public class FirstFitAlgorithm implements ISpectrumAssignmentAlgorithm{ // NOPMD by Andr� on 07/06/17 09:26
	/**
	 * Inst�ncia da Classe.
	 * @author Andr� 			
	 */		
	private static final FirstFitAlgorithm FF_INSTANCE = new FirstFitAlgorithm();
	/**
	 * M�todo para encontrar um conjunto de slots de frequencia
	 * cont�nuos e cont�guos.
	 * @author Andr� 			
	 */				
	public List<Integer> findFrequencySlots(final int numberOfSlots, final int reqNumbOfSlots, final List<OpticalLink> uplink, final List<OpticalLink> downlink) throws InvalidNumberOfFrequencySlotsException, InvalidRoutesException { // NOPMD by Andr� on 05/06/17 13:08
		
		if(reqNumbOfSlots<=0){
			throw new InvalidNumberOfFrequencySlotsException("Required number of frequency slots is invalid", 1);
		}else if (uplink.isEmpty()){
			throw new InvalidRoutesException("Routing solution is invalid", 3);			
		}		
	 ...
	 * @return O objeto FF_INSTANCE
	 * @author Andr� 			
	 */	
	public static FirstFitAlgorithm getFFInstance(){
		return FF_INSTANCE;
	}
}
