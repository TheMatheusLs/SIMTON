...
import types.GainAlgorithmType;
...
public final class GainAlgorithm {
	...
	private void configureGainInLink(final OpticalLink link) {
		...
		final double fiberLoss = -link.getLength()*parameters.getFiberAtenuationCoefficient();	// km * (dB/ Km)  = dB
		final double connectorsLoss = (double) (spansSize*(-2.0*parameters.getDioLoss()));		// dB
		final double nodesLoss = -parameters.getMuxLoss()*2.0-parameters.getSwitchLoss();		// dB
